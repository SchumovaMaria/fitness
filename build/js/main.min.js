...
window.addEventListener('DOMContentLoaded', function () {
  ...
  window.addEventListener('load', function () {
    Object(_modules_modals_init_modals__WEBPACK_IMPORTED_MODULE_1__["initModals"])(); // Удаление классов отключённого js
    ...
    var videoPoster = document.querySelector('[data-video-poster]');

    if (videoButton && videoPlayer && videoPoster) {
      var hideVideoPoster = function hideVideoPoster() {
        setTimeout(function () {
    ...
    var removeTabsContentsActiveClass = function removeTabsContentsActiveClass(tabsButtons, tabsContents) {
    ...
    var setTabsEventListener = function setTabsEventListener(tabsButtons, tabsContents) {
    ...
    var tabsButtons = Array.from(document.querySelectorAll('[data-tab-button]'));
    var tabsContents = Array.from(document.querySelectorAll('[data-tab-content]'));
    setTabsEventListener(tabsButtons, tabsContents); // Слайдер

    var SliderDirections = {
      PREVIOUS: 'prev',
      NEXT: 'next'
    };
    var tabletMediaQuery = window.matchMedia('(max-width: 1199px)');
    var mobileMediaQuery = window.matchMedia('(max-width: 767px)');

    var updateSlidesNum = function updateSlidesNum() {
      var slidesNum = 4;
    ...
    var slidersContainersArray = Array.from(document.querySelectorAll('[data-slider]'));
    var slidersArray = [];

    var getSliders = function getSliders(slidesNum) {
      if (slidersContainersArray && slidersContainersArray.length > 0) {
        var _loop2 = function _loop2(i) {
          ...
          slidersArray[i].showSlide = function (direction) {
        ...
        for (var i = 0; i < slidersContainersArray.length; i++) {
          _loop2(i);
        }
      }
    };

    var setSliderButtonsAbility = function setSliderButtonsAbility(slider) {
    ...
    var updateSliderButtonsAbility = function updateSliderButtonsAbility(slider) {
    ...
    var setSliderCardsVisibility = function setSliderCardsVisibility(slider) {
      for (var j = slider.firstVisibleSlideIndex; j <= slider.lastVisibleSlideIndex; j++) {
        slider.slidesCollection[j].style.display = 'grid';
      }

      if (slider.slidesCollection.length > slider.numberOfSlides) {
        for (var _j = slider.numberOfSlides; _j < slider.slidesCollection.length; _j++) {
          slider.slidesCollection[_j].style.display = 'none';
        }
      }
    };

    var increasePossibleClicks = function increasePossibleClicks(slider, direction) {
      if (direction === SliderDirections.PREVIOUS) {
    ...
    var requestSlide = function requestSlide(slider, direction) {
    ...
    var checkDirection = function checkDirection(slider) {
    ...
    var setSliderEventListeners = function setSliderEventListeners(slider) {
      slider.sliderPrevButton.addEventListener('click', function () {
        return requestSlide(slider, SliderDirections.PREVIOUS);
      });
      slider.sliderNextButton.addEventListener('click', function () {
        return requestSlide(slider, SliderDirections.NEXT);
      });
    ...
    var setSliders = function setSliders(slidesNum) {
    ...
    var updateSliders = function updateSliders(slidesNum) {
      getSliders(slidesNum);
    ...
    var currentSlidesNum = updateSlidesNum();
    setSliders(currentSlidesNum);
    window.addEventListener('resize', function () {
      var newSlidesNum = updateSlidesNum();
